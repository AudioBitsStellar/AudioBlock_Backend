...
| **Out-of-order dependency** — `1715000000000-AddEntities` ran *before* `CreateInitialSchema` (users didn’t exist yet) so its FK to `users` always failed. | `1715000000000-AddEntities.ts` | Renamed to `1719619200002-AddEntities.ts` (after initial) and updated `name` property. Old `dist/` artifact removed via `rm -rf dist`. |
