...
describe("WebhookService", () => {
  ...
  beforeEach(() => {
    ...
    mockRepo = {
      ...
      save: jest.fn(async (data) => ({ ...data, id: data.id || "sub-id", createdAt: new Date(), updatedAt: new Date() })),
      find: jest.fn(async () => []),
  ...
  describe("registerSubscription", () => {
    it("allows third party to register a webhook URL", async () => {
      ...
      const sub = await svc.registerSubscription("user-1", "https://example.com/webhook", ["song.minted"], "secret123");
      expect(mockRepo.create).toHaveBeenCalledWith(expect.objectContaining({ endpoint: "https://example.com/webhook" }));
    ...
    it("rejects invalid URLs", async () => {
      const svc = new WebhookService(fetchMock);
      await expect(svc.registerSubscription("user-1", "not-a-url", ["song.minted"])).rejects.toThrow("Invalid endpoint URL");
    });

    it("generates a secret if not provided and stores eventTypes", async () => {
      const svc = new WebhookService(fetchMock);
      const sub = await svc.registerSubscription("user-1", "https://example.com/hook", ["sale.completed"]);
  ...
  describe("signPayload & verify", () => {
    it("creates valid HMAC-SHA256 signature and verifies correctly (signed event payloads)", async () => {
      const svc = new WebhookService(fetchMock);
      const payload = { eventId: "evt-1", eventType: "song.minted", timestamp: new Date().toISOString(), songId: "s1" };
  ...
  describe("deliver with retry and backoff", () => {
    it("retries with exponential backoff on failure and eventually succeeds", async () => {
      ...
      const svc = new WebhookService(fetchMock);
      const payload: any = { eventId: "e1", eventType: "song.minted", timestamp: new Date().toISOString() };
    ...
    it("throws after exhausting max retries (dead letter)", async () => {
      ...
      const svc = new WebhookService(fetchMock);
      const payload: any = { eventId: "e2", eventType: "sale.completed", timestamp: new Date().toISOString() };
      await expect(svc.deliver("https://example.com/hook", payload, "secret")).rejects.toThrow("Webhook delivery failed after 3 attempts");
      expect(fetchMock).toHaveBeenCalledTimes(3);
    });
    ...
    it("succeeds on first try without retry", async () => {
      ...
      const svc = new WebhookService(fetchMock);
      const payload: any = { eventId: "e3", eventType: "song.minted", timestamp: new Date().toISOString() };
      await svc.deliver("https://example.com/hook", payload, "secret");
  ...
  describe("publish filtering", () => {
    it("delivers only to subscriptions matching eventType", async () => {
      const sub1 = { id: "1", endpoint: "https://a.com/hook", secret: "s1", eventTypes: ["song.minted"], isActive: true };
      const sub2 = { id: "2", endpoint: "https://b.com/hook", secret: "s2", eventTypes: ["sale.completed"], isActive: true };
      const subAll = { id: "3", endpoint: "https://c.com/hook", secret: "s3", eventTypes: ["*"], isActive: true };
  ...
  describe("list and delete", () => {
    it("lists subscriptions for a user", async () => {
      ...
      expect(mockRepo.find).toHaveBeenCalledWith({ where: { userId: "u1" }, order: { createdAt: "DESC" } });
    ...
    it("throws when trying to delete non-owned subscription", async () => {
      ...
      const svc = new WebhookService(fetchMock);
      await expect(svc.deleteSubscription("u1", "missing")).rejects.toThrow("Webhook subscription not found");
    });
  });
});
