...
describe('TakedownService — dedicated copyright takedown workflow (reversible unpublish)', () => {
  ...
  describe('createRequest — tracked separately from ContentReport', () => {
    it('creates a takedown request distinct from general moderation (separate entity)', async () => {
      ...
      expect(mockLogRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'takedown_request_created' }),
      );
    });
    ...
    it('throws 404 if song not found', async () => {
      ...
      const svc = new TakedownService();
      await expect(
        svc.createRequest('user-1', 'missing', TakedownReason.COPYRIGHT),
      ).rejects.toMatchObject({ message: 'Song not found' });
    });
  ...
  describe('review workflow — approve (unpublish) and reverse (republish)', () => {
    it('approve temporarily unpublishes song (flagged=true) pending review', async () => {
      ...
      expect(mockLogRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'takedown_approved' }),
      );
    });
    ...
    it('reverse is reversible — republishes song if claim resolved in artist favor', async () => {
      const song: any = {
        ...
        flaggedAt: new Date(),
        flaggedBy: 'admin-1',
        flagReason: 'takedown:td-1:copyright:Valid claim',
      };
      mockSongRepo.findOneBy.mockResolvedValue(song);
      const takedown: any = {
        id: 'td-1',
      ...
      const svc = new TakedownService();
      const result = await svc.reviewRequest(
        'td-1',
        'admin-1',
        'reverse',
        'Claim invalid — artist proved ownership',
      );
      ...
      expect(mockLogRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'takedown_reversed' }),
      );
    });
    ...
    it('throws if trying to reverse non-approved takedown', async () => {
      ...
      mockSongRepo.findOneBy.mockResolvedValue(song);
      mockTakedownRepo.findOne.mockResolvedValue({
        id: 'td-1',
        songId: 's1',
        status: TakedownStatus.PENDING,
      } as any);
      const svc = new TakedownService();
      await expect(svc.reviewRequest('td-1', 'admin-1', 'reverse')).rejects.toThrow(
        'Only approved takedowns can be reversed',
      );
    });
  });
  ...
  describe('list and get', () => {
    it('lists takedown requests', async () => {
      ...
      expect(mockTakedownRepo.find).toHaveBeenCalledWith(
        expect.objectContaining({ where: { status: TakedownStatus.PENDING } }),
      );
    ...
    it('gets single request or throws 404', async () => {
      mockTakedownRepo.findOne.mockResolvedValue(null);
      const svc = new TakedownService();
      await expect(svc.getRequest('missing')).rejects.toMatchObject({
