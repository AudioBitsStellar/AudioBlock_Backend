...
describe('EmbedService — lightweight public player (no auth, rate-limited)', () => {
  ...
  describe('getSongEmbed', () => {
    it('works for public/ready songs without authentication', async () => {
      ...
      mockUserRepo.findOne.mockResolvedValue({
        ...
        name: 'Artist Name',
        username: 'artist1',
        profileImage: 'https://s3.example.com/artist.jpg',
      });
    ...
    it('rejects non-ready or flagged songs (404)', async () => {
      ...
      const svc = new EmbedService();
      await expect(svc.getSongEmbed('song-2', '1.2.3.4')).rejects.toMatchObject({
      ...
      await expect(svc.getSongEmbed('song-3', '1.2.3.4')).rejects.toMatchObject({
        message: 'Song not available',
      });

      mockSongRepo.findOne.mockResolvedValue(null);
      await expect(svc.getSongEmbed('missing', '1.2.3.4')).rejects.toMatchObject({
        message: 'Song not available',
      });
    });
    ...
    it('returns minimal data needed for embeddable player widget', async () => {
      ...
      mockUserRepo.findOne.mockResolvedValue({
        ...
        name: 'Widget Artist',
        username: 'widget',
        profileImage: 'https://cdn.example.com/artist.jpg',
      } as any);
      const svc = new EmbedService();
  ...
  describe('getAlbumEmbed (playlist)', () => {
    it('returns album with embedded songs', async () => {
      ...
      mockSongRepo.findOne
        .mockResolvedValueOnce({
          id: 'song-1',
          title: 'S1',
          coverArtPath: 'https://s3.example.com/c1.jpg',
          artistId: 'artist-1',
          status: 'ready',
          flagged: false,
        } as any)
        .mockResolvedValueOnce({
          id: 'song-2',
          title: 'S2',
          coverArtPath: 'https://s3.example.com/c2.jpg',
          artistId: 'artist-1',
          status: 'ready',
          flagged: false,
        } as any);
    ...
    it('throws 404 for missing album', async () => {
      ...
      const svc = new EmbedService();
      await expect(svc.getAlbumEmbed('missing', '1.2.3.4')).rejects.toMatchObject({
